...
Instead of asking a generic LLM that might hallucinate, DocMind retrieves the most relevant chunks from *your specific documents* and feeds them as context to Qwen 3.8 27B — so every answer is grounded in what's actually written in your files, with exact page-level citations.
...
| **Groq API** | Ultra-fast Qwen 3.8 27B inference |
...
LangChain RetrievalQA → Prompt Template + Context + Question
    │
    ▼
Groq API (Qwen 3.8 27B) → Streamed tokens via SSE → Frontend
```
...
2. The prompt is sent to **Groq API** (Qwen 3.8 27B)
